#!/usr/bin/env python3
"""
Update Main Events with PATCH
Update existing fights with correct main event identification using PATCH
"""
...
def update_main_events():
    ...
    try:
        logger.info("🔧 Updating main event identification with PATCH...")
        ...
        for event_id, main_events in main_event_updates.items():
            ...
            if fights_response.status_code == 200:
                ...
                # First, reset all main event flags to False
                for fight in fights:
                    fight_id = fight['id']
                    
                    # Reset main event flags
                    ...
                    if reset_response.status_code != 204:
                        logger.warning(f"      ⚠️ Failed to reset fight {fight_id}: {reset_response.status_code}")
                
                # Then, set the correct main events
                for fight in fights:
                    fight_id = fight['id']
