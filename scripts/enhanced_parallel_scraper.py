#!/usr/bin/env python3
"""
Enhanced Parallel Scraper
Based on the working script that got 667 events with 1% fail rate
"""
...
class EnhancedParallelScraper:
        ...
        try:
            ...
            for table_idx, table in enumerate(all_tables):
                ...
                if ufc_count > 0:
                    ...
                    table_events = self._extract_events_from_table(table, table_idx)
    ...
    def _extract_events_from_table(self, table, table_idx):
        """Extract events from a specific table"""
        events = []
        ...
        for row_idx, row in enumerate(rows):
            try:
                ...
                if len(cells) >= 3:  # Need at least some basic data
                    ...
                    event_data = self._parse_event_row(cells, table_idx, row_idx)
                    if event_data:
    ...
    def _parse_event_row(self, cells, table_idx, row_idx):
    ...
    def get_or_create_fighter(self, fighter_name):
        ...
        try:
            # Clean fighter name
            fighter_name = self.clean_fighter_name(fighter_name)
            if not fighter_name:
                return None
    ...
    def run_enhanced_parallel_scraper(self, max_events=None, start_from=0):
        """Run the enhanced parallel scraper"""
        logger.info("🚀 Starting Enhanced Parallel Scraper")
        ...
        logger.info(f"\n📊 ENHANCED PARALLEL SCRAPING SUMMARY")
        logger.info("=" * 60)
...
if __name__ == "__main__":
    ...
    # Run the full scraper on ALL events
