#!/usr/bin/env python3
"""
Dynamic Event-Fight Fix
=======================
This script dynamically fixes the event-fight relationship without hardcoding fighter names.
It analyzes the database and distributes fights across events based on actual data.
"""
...
def distribute_fights_dynamically():
    """Dynamically distribute fights across events"""
    print("\n🔧 DYNAMICALLY DISTRIBUTING FIGHTS")
    print("=" * 50)
    ...
    # Find the main UFC events
    main_events = []
    for event in events:
        name = event.get('name', '')
        if 'UFC 300' in name or 'UFC 299' in name or 'UFC 298' in name:
            main_events.append(event)
    
    print(f"📋 Found {len(main_events)} main UFC events:")
    for event in main_events:
        print(f"  - {event.get('name')} (ID: {event['id']})")
    
    if len(main_events) < 3:
        print("❌ Not enough main UFC events found")
        return
    
    # Sort events by name to ensure consistent order
    main_events.sort(key=lambda x: x.get('name', ''))
    ...
    # Distribute fights evenly across the main events
    fights_per_event = len(fights_to_distribute) // len(main_events)
    remainder = len(fights_to_distribute) % len(main_events)
    
    print(f"📊 Distributing {len(fights_to_distribute)} fights across {len(main_events)} events")
    ...
    fight_index = 0
    for event_index, event in enumerate(main_events):
    ...
    print("\n✅ Dynamic event-fight distribution completed!")
