...
"""
Populate Rankings
Populate the scraped rankings back into the database
"""
...
def create_ranking(ranking_data):
    ...
    try:
        ...
        fighter_id = get_or_create_fighter(ranking_data['fighter_name'])
        if not fighter_id:
            logger.warning(f"⚠️ Could not get/create fighter: {ranking_data['fighter_name']}")
            return None
        
        # Create ranking
        ranking_to_create = {
            'fighter_id': fighter_id,
            'weight_class': ranking_data['division'],
            'rank_position': ranking_data['rank'],
            'rank_type': ranking_data['rank_type']
        }
        ...
        if response.status_code == 201:
            ...
            try:
                ...
                fetch_response = requests.get(
                    f"{SUPABASE_URL}/rest/v1/rankings?fighter_id=eq.{fighter_id}&weight_class=eq.{ranking_data['division']}",
                    headers=headers
                )
                if fetch_response.status_code == 200:
...
def populate_rankings():
    ...
    for ranking in rankings:
        try:
            ...
            if ranking_id:
                created_count += 1
                logger.info(f"✅ Created ranking: {ranking['fighter_name']} - {ranking['division']} #{ranking['rank']}")
            else:
                failed_count += 1
                logger.warning(f"❌ Failed to create ranking: {ranking['fighter_name']}")
        except Exception as e:
            failed_count += 1
